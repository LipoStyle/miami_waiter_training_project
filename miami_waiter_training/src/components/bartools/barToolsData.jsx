// barToolsData.js

const barToolsData = [
  {
    name: 'Hawthorne Strainer',
    image: '/images/tools/hawthorne-strainer.jpg',
    description:
      'Το στραγγιχτήρι Hawthorne χρησιμοποιείται σε συνδυασμό με το Koriko tin. Έχει δύο κομμάτια, το χερούλι και το έλασμα. Πιέζοντάς το με τον δείκτη, μπορείς να ελέγξεις την ροή και την υφή των ποτών και των αφρών που παράγονται από το χτύπημα με πάγο.'
  },
  {
    name: 'Fine Strainer',
    image: '/images/tools/fine-strainer.webp',
    description:
      'Σε συνδυασμό με το Hawthorne, το fine αφαιρεί ακόμα περισσότερο πάγο (στην ουσία νερό). Σε ποτά με ασπράδι αφαιρεί τις μεγάλες φυσαλίδες αέρα, κάνοντας τον αφρό πιο πυκνό.'
  },
  {
    name: 'Julep Strainer',
    image: '/images/tools/julep-strainer.webp',
    description:
      'Μία κατασκευή που χρησιμοποιούταν στα Mint Juleps για να κρατάει πίσω τον πάγο μιας και τότε δεν υπήρχαν καλαμάκια. Πλέον χρησιμοποιείται για να σουρώνουμε από Martini Stirring Jar.'
  },
  {
    name: 'Muddler',
    image: '/images/tools/muddler.jpeg',
    description:
      'Στην ουσία ένα ξύλινο ή πλαστικό γουδί για να πρεσσάρεις φρούτα, ζάχαρη, βότανα, μπαχαρικά κλπ. και να απελευθερώσεις γεύσεις και αρώματα.'
  },
  {
    name: 'Trident Spoon / Flathead Bar Spoon / Teardrop Spoon',
    image: '/images/tools/bar-spoon.jpg',
    description:
      'Το καθένα έχει και την λειτουργία του και ο κανόνας είναι πως ανακατεύουμε σε Martini Stirring Jar μόνο με Trident ή Teardrop επειδή είναι ζυγισμένα στο κέντρο. Επίσης το κουτάλι τους είναι μεζούρα 5ml. Το Flathead ή αλλιώς Ευρωπαϊκό, δεν είναι ζυγισμένο και στο πίσω μέρος έχει ένα μικρό muddler το οποίο συνήθως χρησιμοποιούμε για να σπάσουμε ζάχαρη με bitters στα πρώτα στάδια του Old Fashioned. Είναι επίσης το εργαλείο που χρησιμοποιούμε για να κάνουμε ξεκάθαρες στρώσεις υγρών στο ίδιο ποτήρι.'
  },
  {
    name: '25-50 Jigger / Mr. Slim / Thimble Measure',
    image: '/images/tools/jigger.webp',
    description:
      'Το 25-50 έχει επίσης γραμμές 15-35 και το Mr.Slim 5/10/15/20/25/10/20/30/40/50. Με αυτά τα δύο μπορούμε να μετρήσουμε τα πάντα, γρήγορα, και φυσικά να παραδίδουμε πάντα το ίδιο αποτέλεσμα. Τα Thimble, δεν χρησιμοποιούνται για ποτά αλλά για πράγματα όπως μικρή, μέτρια και μεγάλη μερίδα κρασί 125/175/250ml.'
  },
  {
    name: 'Koriko Tin Shaker',
    image: '/images/tools/koriko-tin.webp',
    description:
      'Σε αντίθεση με τα γυάλινα shaker, το Koriko μας επιτρέπει να ελέγχουμε την θερμοκρασία του ποτού κατά την διάρκεια του χτυπήματος και με τα δύο χέρια.'
  },
  {
    name: 'Peeler',
    image: '/images/tools/peeler.jpg',
    description:
      'Στο bar χρησιμοποιείται στην δημιουργία γρήγορης γαρνιτούρας από διάφορες φλούδες π.χ. Πορτοκάλι, λεμόνι, λάιμ, μήλο κλπ.'
  },
  {
    name: 'Nutmeg Grater',
    image: '/images/tools/nutmeg-grater.jpg',
    description:
      'Αν και η δημοτικότητα των Flip έχει πέσει παγκοσμίως, το ξύσμα από μοσχοκάρυδο είναι διαθέσιμο σε όλα τα σύγχρονα bar τιμής ένεκεν των πρωτοπόρων που σκέφτηκαν πως να καλύψουν τις άσχημες μυρωδιές.'
  },
  {
    name: 'Mexican Elbow',
    image: '/images/tools/mexican-elbow.jpeg',
    description:
      'Στίφτης φρέσκων εσπεριδοειδών σε περίπτωση που δεν έχουμε έτοιμο χυμό.'
  },
  {
    name: 'Measuring Spoons',
    image: '/images/tools/measuring-spoons.webp',
    description:
      'Σύνολο μικρών μεζούρων για ακρίβεια δόσεων.'
  },
  {
    name: 'Bar Blade',
    image: '/images/tools/bar-blade.webp',
    description:
      'Ανοιχτήρι μπουκαλιών μπύρας, τόνικ κλπ. Η τρύπα στο πίσω μέρος βοηθάει να το τραβήξουμε από τις πίσω τσέπες με άνεση και ταχύτητα. Είναι πολύ σύνηθες να είναι προσωποποιημένα στον/ην κάθε bartender.'
  },
  {
    name: "Waiter's Friend",
    image: '/images/tools/waiters-friend.jpg',
    description:
      'Ανοιχτήρι μπουκαλιών κρασιού.'
  },
  {
    name: "Store n' Pour",
    image: '/images/tools/store-pour.jpg',
    description:
      'Συνήθως χρησιμοποιείται για αποθήκευση και διοχέτευση υγρών με μεγάλες καταναλώσεις.'
  },
  {
    name: 'Squeezee',
    image: '/images/tools/squeezee.jpg',
    description:
      'Αντίστοιχο με το Store n\' Pour αλλά με την έξτρα δυνατότητα συμπίεσης για να χρησιμοποιείται σε παχύρρευστα υγρά όπως το μέλι.'
  },
  {
    name: 'Vintage Dash Bottle / Square Dash',
    image: '/images/tools/dash-bottle.webp',
    description:
      'Αποθήκευση και διοχέτευση υγρών με μικρές καταναλώσεις όπως αψέντι και “σπιτικά” bitters.'
  },
  {
    name: 'Pour Spout',
    image: '/images/tools/pour-spout.webp',
    description:
      'Υπάρχουν δύο μεγέθη. Το μικρό που χρησιμοποιούμε στα μπουκάλια αλκοόλ και το μεγάλο για τα πιο παχύρρευστα όπως τα σιρόπια.'
  }
];


export default barToolsData;
